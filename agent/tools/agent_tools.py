...
def get_weather(city: str):
    return f"{city}为晴天，天气26摄氏度，空气湿度50%，南风1级，AQI21，最近六小时降雨概率低"

@tool(description="获取用户所在的城市名")
def get_user_location():
    return random.choice(["深圳","北京","合肥"])
...
@tool(description="从外部系统中获取用户在指定月份的使用记录，以纯字符串返回，未检测到则返回空字符串")
def fetch_external_data(user_id: str, month: str):
    generate_external_data()
    try:
        return external_data[user_id][month]
    except KeyError:
        logger.warning(f"[fetch_external_data]未能检索到用户id在该月的使用记录")
        return ""
